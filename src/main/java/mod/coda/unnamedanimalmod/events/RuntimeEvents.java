    ...
    public static void fruitRageAIGoal(EntityJoinWorldEvent event) {
        if (event.getEntity() instanceof MobEntity) {
                ...
                protected void findNearestTarget() {
                    nearestTarget = goalOwner.world.getClosestEntity(
                            goalOwner.world.getEntitiesWithinAABB(
                                    ...
                                    e -> goalOwner.getActivePotionEffect(UAMEffects.FRUIT_RAGE.get()) != null && !e.getType().equals(goalOwner.getType())),
            ...
            if (entity instanceof CreatureEntity) {
                entity.goalSelector.addGoal(1, new MeleeAttackGoal((CreatureEntity) entity, 2.0D, true));
            }

            if (!entity.getAttributeManager().hasAttributeInstance(Attributes.ATTACK_DAMAGE)) {
                entity.getAttribute(Attributes.ATTACK_DAMAGE).applyPersistentModifier(new AttributeModifier("Attack damage", 2, AttributeModifier.Operation.ADDITION));
            }
        }
    }
