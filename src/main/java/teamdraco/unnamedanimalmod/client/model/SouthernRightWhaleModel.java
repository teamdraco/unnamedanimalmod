    ...
    public void setupAnim(SouthernRightWhaleEntity entityIn, float f, float f1, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (Entity.getHorizontalDistanceSqr(entityIn.getDeltaMovement()) > 1.0E-7D) {
            if(!entityIn.isInWater() && !entityIn.isOnGround()) {
                ...
                this.tail_stock2.xRot = MathHelper.cos(-2.0F + f * speed * 0.4F) * degree * 0.5F * f1;
                this.flukes.xRot = MathHelper.cos(-2.0F + f * speed * 0.4F) * degree * 0.8F * f1;
                this.head.xRot = MathHelper.cos(1.0F + f * speed * 0.4F) * degree * 0.1F * f1;
                this.jaw.xRot = MathHelper.cos(1.0F + f * speed * 0.4F) * degree * 0.1F * f1 + 0.05F;
            }
        }
        else {
            this.body.zRot = 0;
